...
export const useTableAStore = defineStore({
  ...
  state: () => ({
    columns: [
      { id: 'name', title: 'Name', width: 90, align: 'left', fixed: 'left', sortable: false, autoScale: true },
      ...
      { id: 'status', title: 'Status', width: 60, align: 'right', sortable: true, autoScale: true },
  ...
  actions: {
    ...
    generateMockData() {
      ...
      return Array.from({ length: fetchLength }, (_, i) => ({
        ...
        name: `UserA ${(this.currentPage - 1) * this.pageSize + i}`,
